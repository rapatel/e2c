public class Parser {
    ...
    private void bcGen() {
    	gcprint("int bc(int x, int size, int line, char* str, int lbound, int ubound)");
    	gcprint("{");
    	gcprint("if(x<size && x>=0) {");
    	gcprint("return x;");
    	gcprint("} else {");
    	gcprint("fprintf(stderr, \"subscript (%d) out of bounds for array %s[%d:%d] on line %d\\n\", x+lbound, str, lbound, ubound, line);");
    	gcprint("exit(1);");
    	gcprint("}");
    	gcprint("}");
    }
    ...
    private void forloop(boolean i, boolean r) {
    	...
    	if(is(TK.ID)) {
    		...
    		if(e.arrSize == 0) {
    			parse_error("right-hand-side of : in every isn't array " + tok.string);
    		}
    	} else {
