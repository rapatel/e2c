...
public class Scan {
	...
	public Token scan() {
		...
		while (true) {
			...
			// character in the input stream, therefore, when we first enter
			// scan(), do not call getchar().
			// ANSWER 2: putback tells us that we have read an extra character
			// in functions that are reading an ID, NUM, Comment, Divide/NE.

			// QUESTION 2: What are the values of putback and c just before the
			// identifier
			...
			if (myisalpha((char) c)) {
				...
				case '-':
					...
					// QUESTION 3: What does the following case and the code in
					// it do?
					// ANSWER 3: The following checks whether the next token
					// type is DIVIDE or NOT EQUAL. The scanner already knows
					// that
					// the current character is a '/', so it must read the next
					// char. If the next char read is not a '=', then the
					// scanner
					// has read an extra char (and sets putback to true) and
					// knows it is a DIVIDE token. Otherwise, it is a NOT EQUAL
					// token.
				case '/':
				...
				case ']':
					return new Token(TK.ENDARR, new String(String.valueOf(c)),
							linenumber);
